...
`FastLog` is a simple .NET logging library built around DI (`Microsoft.Extensions.DependencyInjection`).
...
## Installation (in this solution)

The example project uses a local reference:

```xml
<ItemGroup>
  <ProjectReference Include="..\FastLog\FastLog.csproj" />
</ItemGroup>
```
...
- `FileLoggingPath` (string?): log file path (for example `logs/fastlog.log`)
...
public class App
{
	...
	public void Run()
	{
		...
		_fastLog.Log(LogType.Warning, "This is a warning log");
	}
}
```
